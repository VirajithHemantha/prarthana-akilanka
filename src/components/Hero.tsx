...
export const Hero: React.FC = () => {
  ...
  const y1 = useTransform(scrollY, [0, 1000], touchDevice ? [0, 0] : [0, 400]);
  const scale = useTransform(scrollY, [0, 800], touchDevice ? [1, 1] : [1, 1.1]);
  const opacity = useTransform(scrollY, [0, 400], touchDevice ? [1, 1] : [1, 0]);
  ...
  return (
    <div ref={containerRef} className="relative min-h-[100dvh] h-[100dvh] flex items-center justify-center overflow-hidden bg-brand-ivory/50">
      ...
      >
        ...
        <img 
          src="/WhatsApp Image 2026-06-25 at 19.45.46.jpeg" 
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-brand-ivory via-brand-ivory/30 to-brand-ivory/50 pointer-events-none" />
        <div className="absolute inset-0 bg-gradient-to-b from-brand-beige/15 via-transparent to-brand-beige/10 mix-blend-overlay pointer-events-none" />
      </motion.div>
        ...
        >
          ...
          <div className="relative mb-8 sm:mb-12 w-full flex justify-center">
            ...
            <h1 className="relative text-6xl sm:text-[7rem] lg:text-[9.5rem] font-display text-stone-800 leading-[1.1] sm:leading-[0.9] drop-shadow-sm">
              Prarthana <br className="sm:hidden" />
      ...
      <div className="absolute left-6 sm:left-10 top-1/2 -translate-y-1/2 hidden lg:flex flex-col items-center gap-6 mix-blend-multiply opacity-70">
        ...
        <p className="writing-mode-vertical text-[11px] uppercase tracking-[0.6em] text-brand-beige-deep font-semibold font-sans">
          Asliya Regency Hotel • Kandy Road Pilessa
        </p>
      ...
      <motion.button
        ...
        className="absolute bottom-8 sm:bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 cursor-pointer bg-transparent border-0 p-0"
